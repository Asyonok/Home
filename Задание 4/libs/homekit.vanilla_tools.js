...
function showNotification(text,typeselector) {
  var obj = ['info','success','warning','error','notify'];

  if (obj.indexOf(typeselector) != -1) {
    vNotify[typeselector]({text: text});
  }
  else {console.log('ERR: type of vNotify is not correct')
...
function send(method, url, params, ecb, scb) {
  ...
  if(user_info.token)
    url += url.indexOf('?') >-1 ? '&access_token=' + user_info.token :'?access_token=' + user_info.token;
  ...
  // if(user_info.token)
  //   xhr.setRequestHeader('access_token', user_info.token);
